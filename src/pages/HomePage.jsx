...
export default function HomePage() {
  ...
  return (
    <section className="relative overflow-hidden max-w-5xl mx-auto px-6 py-8 md:py-12 flex flex-col gap-8">
      {/* Gradient glow accent */}
      <div
        className="pointer-events-none absolute -top-40 -right-40 w-[500px] h-[500px] rounded-full blur-3xl opacity-[0.07]"
        style={{ background: '#2898c1' }}
      />
      {/* Hero row */}
      <div className="flex flex-col-reverse md:flex-row items-center gap-8">
        <AnimatedSection className="flex-1 text-center md:text-left">
          <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold text-text font-heading mb-1">
            Hi, I'm <span className="text-primary">Gabriel Kiveu</span>!
          </h1>
          <p className="font-mono text-sm text-primary/70 tracking-[0.15em] uppercase mb-4">
        ...
        <AnimatedSection className="flex-shrink-0" delay={200}>
          <img
            ...
            alt="Gabriel Kiveu"
            className="w-56 h-56 md:w-64 md:h-64 rounded-2xl object-cover border-2 border-border ring-4 ring-primary/20 shadow-2xl"
          />
        </AnimatedSection>
      </div>
